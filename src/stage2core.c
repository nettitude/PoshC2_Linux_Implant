    ...
    *response = malloc(response_allocated); // FREE
    ...
    if ((capture_stderr == CAPTURE_STDERR) && (stderr_buf_len > 0)) {
        ...
        if (*response_len + (stderr_buf_len + strlen(stderr_header)) > response_allocated) {
            *response = realloc(*response, response_allocated + stderr_buf_len + 1 +
                                           strlen(truncation_msg)); // slightly wasteful to include the truncation message, but saves a potential realloc below
            if (*response == NULL) {
                return -1;
            }
